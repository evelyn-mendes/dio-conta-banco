...
## 📖 Sobre
A aplicação coleta dados como nome, agência, número da conta e o saldo, do usuário, e imprime após terminar de coletar as informações. Está aplicação foi feita utilizando apenas a linguagem `Java` e a IDE `VSCODE`.
...
``` bash
    ...
    $ git clone https://github.com/evelyn-mendes/dio-conta-banco.git
```
...
### Problema:
- Ao colocar o ponto para colocar o saldo, ocorreu um erro, isso pode ocorrer por conta das configurações regionais do `sistema operacional`.
...
## 📖 About
The application collects data such as the name, agency, account number, and balance of the user and then prints the collected information. It was made using the language `Java` and the `VSCODE` IDE.
